...
namespace 医药管理系统
{
    public partial class sales_select2 : Form
    {
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            if (textBox1.Text == "请输入订单日期、客户、订单号、负责人")
            {
                ...
                {
                    con.Open();
                    cmd = new MySqlCommand("select * from sale1 where remark not like '%退单%' and state = '待出库'", con);
            ...
            else
            {
                ...
                try
                {
                    ...
                    while (reader1.Read())
                    {
                        if (reader1[6].ToString().Contains("退单") == false&&reader1[5].ToString()=="待出库")
                        {
                            int x = dataGridView1.RowCount;
                            dataGridView1.Rows.Add();
